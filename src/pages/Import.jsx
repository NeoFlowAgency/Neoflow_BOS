...
export default function Import() {
  ...
  const [aiMapping, setAiMapping]   = useState(null)       // { mappings, explanation }
  ...
  const handleImport = async () => {
    ...
    const rows = mappedRows.filter(r => r._errors.length === 0)
        ...
        {step < 4 && (
          <div className="flex items-center justify-between mt-4">
            ...
            >
              ...
              {step === 3
                ? `Importer ${mappedRows.filter(r => !r._errors?.length).length} enregistrement(s)`
                : 'Suivant →'
              }
            </button>
          </div>
