...
export default function Sidebar({ isOpen, setIsOpen }) {
  ...
  const NavGroup = ({ icon, label, isExpanded, onToggle, children }) => (
    <div>
      <button
        onClick={onToggle}
        className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl font-medium transition-all text-gray-600 hover:bg-gray-100/80 hover:text-gray-900 ${
  ...
  const MobileBottomNav = () => {
    ...
    <>
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${
                    isActive ? 'bg-[#313ADF] text-white' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900'
                  }`
                }
              >
                <span className="flex-shrink-0 relative">
      ...
      {/* Nav bar */}
      <div
        className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/85 backdrop-blur-2xl border-t border-gray-200/60"
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 h-screen bg-white/80 backdrop-blur-2xl border-r border-gray-200/80 flex flex-col p-5 transition-all duration-300 z-40
          ${isMobile
      ...
      >
        ...
        {isMobile && (
          <button
            onClick={() => setIsOpen(false)}
            className="absolute top-4 right-4 p-2 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-xl transition-colors z-10"
        ...
        {currentWorkspace && (isOpen || isMobile) && (
          <div className="mb-4 relative" ref={wsDropdownRef}>
            <button
              ...
              className="w-full px-3 py-2 bg-gray-100/80 rounded-xl hover:bg-gray-100 transition-colors text-left"
            >
            ...
            {wsDropdownOpen && (
              <div className="absolute left-0 right-0 top-full mt-1 bg-white border border-gray-200 rounded-xl shadow-xl z-50 overflow-hidden">
                {workspaces.map(ws => (
                  ...
                  >
                    <span className="w-6 h-6 bg-gray-100 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0">
        ...
        {currentWorkspace && !isOpen && !isMobile && (
          <button
            onClick={() => setWsDropdownOpen(!wsDropdownOpen)}
            className="mb-4 w-10 h-10 mx-auto bg-gray-100/80 rounded-xl flex items-center justify-center hover:bg-gray-100 transition-colors"
            title={currentWorkspace.name}
          >
            <span className="text-gray-900 text-sm font-bold">{currentWorkspace.name?.charAt(0)?.toUpperCase()}</span>
          </button>
        )}
