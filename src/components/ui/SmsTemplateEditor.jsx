...
const ALL_VARIABLES = [
  { key: 'prenom',    label: 'Prénom',          example: 'Sophie' },
  { key: 'numero',    label: 'N° commande',      example: 'CMD-2026-042' },
  { key: 'magasin',   label: 'Magasin',          example: 'Ma Literie' },
  { key: 'date',      label: 'Date livraison',   example: '14 mai' },
  { key: 'creneau',   label: 'Créneau horaire',  example: '10h–12h' },
  { key: 'lien_avis', label: 'Lien avis Google', example: 'g.page/r/…' },
]
...
function renderPreview(text) {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (varDef) {
      parts.push(
        <span key={i++} className="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold bg-[#313ADF]/10 text-[#313ADF] border border-[#313ADF]/20">
          {varDef.example}
        </span>
      )
    } else {
          ...
          <div className="flex flex-wrap gap-1.5">
            {vars.map(v => (
              <button
                ...
                className="px-2.5 py-1 rounded-lg text-xs font-semibold border cursor-pointer hover:opacity-75 transition-opacity bg-gray-100 text-gray-600 border-gray-200 hover:bg-gray-200"
              >
                + {v.label}
              </button>
            ))}
